...
System został zaprojektowany jako rozwiązanie łączące analizę danych populacyjnych z mechanizmami wyszukiwania literatury naukowej oraz generowania tekstu przy użyciu dużych modeli językowych. Architektura systemu składa się z modułów odpowiedzialnych za przygotowanie danych, budowę bazy wiedzy, wyszukiwanie publikacji naukowych oraz generowanie raportów.

Głównym założeniem architektury było rozdzielenie procesu przygotowania bazy wiedzy od procesu generowania raportów. Kosztowne operacje związane z pozyskiwaniem publikacji, przetwarzaniem ich treści oraz generowaniem reprezentacji wektorowych wykonywane są jednorazowo podczas budowy bazy wiedzy, co znacząco skraca czas generowania raportów.

![Diagram architektury systemu](figs/Diagram%20architektury%20systemu.png){#fig-arch width=80% fig-pos='H'}

System składa się z dwóch głównych części: procesu budowy bazy wiedzy oraz procesu generowania raportów. Diagram przepływu przedstawiony jest na @fig-arch. Rozdzielenie tych etapów pozwala na jednorazowe przygotowanie zasobów wiedzy wykorzystywanych później podczas generowania raportów dla użytkownika.
...
Kolejnym etapem jest utworzenie reprezentacji wektorowych dla treści publikacji [@openai_embeddings]. Wygenerowane embeddingi wraz z metadanymi zapisywane są w bazie PostgreSQL rozszerzonej o mechanizm pgvector [@postgresql_docs, @pgvector_docs]. Tak przygotowana baza wiedzy umożliwia późniejsze wyszukiwanie semantyczne publikacji naukowych na podstawie znaczenia zapytania, a nie wyłącznie dopasowania słów kluczowych [@reimers2019sentencebert].
...
## Technologie i koncepcje wykorzystane w pracy

### Przetwarzanie języka naturalnego

Przetwarzanie języka naturalnego (ang. *Natural Language Processing*, NLP) stanowi dziedzinę sztucznej inteligencji zajmującą się analizą, interpretacją oraz generowaniem tekstu w języku naturalnym przez systemy komputerowe. Metody NLP umożliwiają automatyczne przetwarzanie dużych zbiorów dokumentów tekstowych, ekstrakcję informacji, klasyfikację treści oraz wyszukiwanie semantyczne [@jurafsky2025slp]. W ostatnich latach dynamiczny rozwój modeli opartych na architekturze Transformer znacząco zwiększył możliwości systemów NLP, umożliwiając realizację złożonych zadań wymagających rozumienia kontekstu tekstowego [@vaswani2017attention].

### Reprezentacje wektorowe i embeddingi

W celu umożliwienia komputerowej analizy tekstu dokumenty muszą zostać przekształcone do postaci numerycznej. Współczesne systemy NLP wykorzystują w tym celu reprezentacje wektorowe określane jako embeddingi. Embedding jest wielowymiarowym wektorem liczbowym opisującym znaczenie semantyczne fragmentu tekstu [@openai_embeddings]. Dokumenty o zbliżonej tematyce posiadają podobne reprezentacje wektorowe, co umożliwia realizację wyszukiwania semantycznego [@reimers2019sentencebert]. Embeddingi stanowią podstawę działania systemów Retrieval-Augmented Generation oraz wektorowych baz danych [@lewis2020rag; @pgvector_docs].

### Duże modele językowe

Duże modele językowe (ang. *Large Language Models*, LLM) są modelami sztucznej inteligencji trenowanymi na bardzo dużych zbiorach tekstów. Ich zadaniem jest modelowanie zależności występujących w języku naturalnym oraz generowanie spójnych odpowiedzi na podstawie dostarczonego kontekstu [@brown2020gpt3]. Współczesne modele językowe, takie jak GPT, wykorzystują architekturę Transformer, która umożliwia efektywne modelowanie długodystansowych zależności pomiędzy elementami tekstu [@vaswani2017attention]. Modele te znajdują zastosowanie między innymi w systemach dialogowych, wyszukiwaniu informacji, analizie dokumentów oraz wspomaganiu podejmowania decyzji.

### Architektura Retrieval-Augmented Generation

Retrieval-Augmented Generation (RAG) jest architekturą łączącą mechanizmy wyszukiwania informacji z możliwościami generacyjnymi dużych modeli językowych. W klasycznym podejściu model językowy generuje odpowiedź wyłącznie na podstawie wiedzy zgromadzonej podczas procesu uczenia [@brown2020gpt3]. W architekturze RAG przed wygenerowaniem odpowiedzi wykonywane jest dodatkowe wyszukiwanie dokumentów powiązanych semantycznie z zapytaniem użytkownika. Odnalezione dokumenty są następnie przekazywane do modelu językowego jako dodatkowy kontekst, co pozwala ograniczyć problem halucynacji modeli językowych oraz zwiększyć aktualność generowanych odpowiedzi [@lewis2020rag].

## Przegląd istniejących rozwiązań

Interesującym przykładem wykorzystania dużych modeli językowych w obszarze żywienia jest rozwiązanie DietAI24 zaproponowane przez Yan i współautorów [-@yan2025dietai24]. Autorzy opracowali system umożliwiający automatyczną ocenę wartości odżywczej posiłków na podstawie zdjęć żywności. W zaproponowanym podejściu wykorzystano multimodalne duże modele językowe (MLLM) wspomagane architekturą Retrieval-Augmented Generation, która umożliwia odwoływanie się do zewnętrznej bazy wiedzy zawierającej dane żywieniowe pochodzące z Food and Nutrient Database for Dietary Studies (FNDDS). Zastosowanie mechanizmu RAG pozwoliło ograniczyć zależność od wiedzy wewnętrznej modelu oraz zwiększyć dokładność szacowania wartości odżywczych. Autorzy wykazali, że DietAI24 osiąga istotnie lepsze wyniki niż tradycyjne podejścia oparte wyłącznie na metodach komputerowego rozpoznawania obrazu. Przedstawione rozwiązanie potwierdza potencjał integracji dużych modeli językowych z zewnętrznymi źródłami wiedzy w zastosowaniach związanych z analizą żywienia [@yan2025dietai24].

Przykładem zastosowania architektury Retrieval-Augmented Generation w obszarze medycznym jest praca Su i Gu [-@su2024ragtcm], w której wykorzystano mechanizm RAG do ograniczenia uprzedzeń dużego modelu językowego Llama wobec tradycyjnej medycyny chińskiej. W zaproponowanym rozwiązaniu zgromadzono zewnętrzną bazę wiedzy obejmującą recenzowane artykuły naukowe, wyniki badań klinicznych, metaanalizy oraz historyczne teksty dotyczące tradycyjnej medycyny chińskiej. Zebrane dokumenty zostały poddane przetwarzaniu z wykorzystaniem metod NLP, a następnie zaindeksowane przy użyciu mechanizmu wyszukiwania wektorowego FAISS [@johnson2019faiss]. Podczas generowania odpowiedzi model Llama otrzymywał dodatkowy kontekst w postaci dokumentów odnalezionych na podstawie zapytania użytkownika. Wyniki eksperymentów wykazały, że zastosowanie architektury RAG pozwoliło zwiększyć dokładność generowanych odpowiedzi, ograniczyć występowanie błędnych informacji oraz zmniejszyć wpływ uprzedzeń obecnych w danych treningowych modelu. Autorzy podkreślają również znaczenie wykorzystania aktualnych i wiarygodnych źródeł wiedzy zewnętrznej, które pozwalają poprawić rzetelność odpowiedzi generowanych przez duże modele językowe [@su2024ragtcm].

Istotnym kierunkiem rozwoju współczesnych systemów wspomagających pracę naukowców jest wykorzystanie metod generatywnej sztucznej inteligencji do automatyzacji procesu wyszukiwania oraz analizy literatury naukowej. Chodak i Filipek [-@chodak2025genai] wskazują, że narzędzia oparte na dużych modelach językowych mogą wspierać badaczy na różnych etapach procesu badawczego, w szczególności podczas wyszukiwania, analizy oraz syntezy publikacji naukowych. Autorzy zwracają uwagę na rosnące znaczenie wyspecjalizowanych narzędzi, takich jak Elicit, Consensus czy NotebookLM, które umożliwiają automatyczne przeszukiwanie literatury, identyfikację najważniejszych informacji oraz generowanie syntetycznych podsumowań wyników badań. Jednocześnie podkreślono konieczność krytycznej oceny wyników generowanych przez modele językowe oraz potrzebę weryfikacji otrzymanych informacji z wykorzystaniem wiedzy eksperckiej. Przedstawione rozwiązania potwierdzają rosnący potencjał metod NLP i dużych modeli językowych w procesie wspomagania analizy literatury naukowej [@chodak2025genai].

Przedstawione rozwiązania potwierdzają rosnące znaczenie dużych modeli językowych oraz architektury Retrieval-Augmented Generation w obszarze żywienia i medycyny. Wspólną cechą opisanych systemów jest wykorzystanie zewnętrznych źródeł wiedzy w celu zwiększenia wiarygodności generowanych odpowiedzi oraz ograniczenia problemu halucynacji modeli językowych. W przeciwieństwie do zaprezentowanych prac, opracowany w niniejszej pracy system integruje dane populacyjne pochodzące z badania NHANES z aktualną literaturą naukową pozyskiwaną z bazy PubMed. Celem proponowanego rozwiązania jest nie tylko identyfikacja problemów związanych ze spożyciem składników odżywczych, ale również automatyczne generowanie raportów interpretujących uzyskane wyniki w kontekście aktualnych dowodów naukowych. Zastosowanie dedykowanej bazy wiedzy oraz mechanizmu oceny jakości publikacji pozwala dodatkowo preferować źródła dostarczające silniejszych dowodów naukowych, co zwiększa rzetelność generowanych raportów.
...
Przygotowana w ten sposób baza wiedzy stanowi fundament procesu Retrieval-Augmented Generation wykorzystywanego przez system [@lewis2020rag]. W momencie generowania raportu odpowiednie publikacje wyszukiwane są na podstawie podobieństwa semantycznego do analizowanego problemu żywieniowego, a następnie przekazywane do modelu językowego jako dodatkowy kontekst wspomagający interpretację wyników analizy.
